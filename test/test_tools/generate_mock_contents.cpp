...
void generate_mock_contents(const MockContentsGeneratorParams& params,
        const Random<int>& prng, const std::string_view content_seed, std::ostream& contents)
{
    ...
    while (nr_reps-- && curr_size < params.size_limit) {
        size_t pos = 0, len = content_seed.size();
        ...
        std::string data {content_seed.substr(pos, len)};
