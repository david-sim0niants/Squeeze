...
TEST_P(Huffman15Test, EncodesDecodesData)
{
    ...
    auto [in_it, en_stat] = huffman15_encoder.template encode_data<false>(data.begin(), data.end());
    ...
    auto [out_it, de_stat] = huffman15_decoder.template decode_data<false>(rest_data.begin(), rest_data.end());
...
TEST_P(Huffman15Test, EncodeDecode)
{
    std::vector<char> data = GetParam()->get_data();
    std::vector<char> buffer;
    auto en_stat = std::get<2>(huffman15_encode(data.begin(), data.end(), std::back_inserter(buffer)));
    ASSERT_TRUE(en_stat.successful()) << en_stat.report();

    const double compression_ratio = data.size() / (double)buffer.size();
    EXPECT_GE(compression_ratio, 1.0);

    std::vector<char> rest_data(data.size());
    auto de_stat = std::get<2>(huffman15_decode(rest_data.begin(), rest_data.end(),
